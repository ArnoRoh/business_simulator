...
docs/                  Durable design and domain documentation.
  ...
  concept-note.html    The programme: intervention, pilot and cost-effectiveness.
  concept-note-model.xlsx  The same model as a workbook, with live formulas.
  concept-note-model.csv   The same calculation, flat.
                       ALL THREE ARE GENERATED. The arithmetic lives in
                       scripts/lib/cea_model.py; run build-concept-note.py and
                       build-model-xlsx.py. Never hand-edit the outputs.
...
memory/                Persistent project memory. READ THIS FIRST. See §5.
                       NOT PUBLISHED — gitignored, lives only on the working machine.
...
### `memory/` is not in the repository

It is gitignored and kept on the machine the work happens on. The reason is in the
content: it records private conversations with the project owner and with partners, it
names people and firms, and it is written candidly on purpose. A log that is written
for a public audience becomes a press release and stops being worth keeping.

Three consequences, all of which matter:

1. **Git is not backing it up.** It is the least replaceable thing in the project and
   the only thing with no remote copy. Back it up separately.
2. **Links from public files into `memory/` will not resolve** for anyone reading the
   repository on GitHub. `scripts/check-links.sh` skips them when the directory is
   absent and checks them when it is present.
3. **Write it as if a stranger will read it, and as if the person you are writing about
   will too.** Record what a partner said and what was rejected — that is the point of
   the log — but do not put a private assessment of a named individual's competence, or
   of a specific firm's numbers, anywhere in this repository. If that view needs
   keeping, it belongs somewhere else entirely.
